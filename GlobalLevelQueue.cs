...
class GlobalLevelQueue {
    ...
    static void Main(string[] args) {
        ...
        // create a queue of labor orders called "laborOrders"
        ...
        // these would usually be generated by zones created by the users or directly by the user through the use of crafting stations
        for (int i = 0; i < 25; i++) {
            int type = new Random().Next(0, 3);
            if (type == 0) {
                laborOrders.Enqueue(new LaborOrder(0, 0, new Random().Next(100, 500), new string[] { }, "storage", i+1));
            } else if (type == 1) {
                laborOrders.Enqueue(new LaborOrder(0, 0, new Random().Next(100, 500), new string[] { }, "forage", i+1));
            } else {
                laborOrders.Enqueue(new LaborOrder(0, 0, new Random().Next(100, 500), new string[] { }, "generic", i+1));
            }
        }
        ...
        while (laborOrders.Count > 0) {
            ...
            // no valid pawns; move this labor order to the back of the queue and continue assigning (skipping this iteration).
            if(pawn == null){

                // move the item at the front of the queue to the back of the queue
                laborOrders.Enqueue(laborOrders.Dequeue());
            ...
            // print out the number of free pawns of each type to console
            //Console.WriteLine("Free Pawns: {0} storage, {1} forage, {2} generic", freePawns.Count(p => p.type == "storage"), freePawns.Count(p => p.type == "forage"), freePawns.Count(p => p.type == "generic"));
            ...
            // this where the LaborOrder would be passed to the PawnAI to process/complete in Unity.
